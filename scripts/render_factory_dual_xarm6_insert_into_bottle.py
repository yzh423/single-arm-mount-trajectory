...
CSV = ROOT / "data/factory/8-12/InsertIntoBottle/handheld_20260812_112945.csv"
OUT = ROOT / "reports/factory_bimanual/insert_into_bottle_dual_xarm6/insert_into_bottle_full_se3_fixed_time_front_720p.mp4"

SELECTED_MOUNT = {
    "xy": {
        "left": (0.14923736405545412, 0.04366977266226578),
        "right": (-0.2602388943344565, 0.39757057960135916),
    },
    "yaw": {"left": 225.0, "right": 315.0},
...
def enforce_paired_collision_safety(model, qpos, time_s, velocity_limit_rad_s=3.14):
    """Execute the closest bounded paired state without state/swept collision."""
    contract = ROBOT_CONTRACTS["xarm6"]
    names = {side: {"joints": contract.prefixed_joint_names(side)}
             for side in ("left", "right")}
    checker = MuJoCoPairedCollisionChecker(
        model, mujoco.MjData(model), names, transition_steps=7)
    qids = {}
    for side in ("left", "right"):
        joint_ids = [mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, name)
                     for name in names[side]["joints"]]
        qids[side] = np.asarray(model.jnt_qposadr[joint_ids], dtype=int)
    desired = np.asarray(qpos, float)
    safe = desired.copy(); modified = np.zeros(len(desired), bool)
    intervals = np.r_[time_s[1]-time_s[0], np.diff(time_s)]
    previous = (desired[0, qids["left"]].copy(), desired[0, qids["right"]].copy())
    if not checker.state(*previous).valid:
        raise RuntimeError("initial paired state is colliding")
    for row in range(1, len(desired)):
        target = (desired[row, qids["left"]], desired[row, qids["right"]])
        limit = velocity_limit_rad_s * intervals[row]
        bounded = tuple(old + np.clip(goal-old, -limit, limit)
                        for old, goal in zip(previous, target))
        options = (bounded, (bounded[0], previous[1]),
                   (previous[0], bounded[1]), previous)
        valid = [item for item in options
                 if checker.state(*item).valid
                 and checker.transition(previous, item).valid]
        selected = min(valid, key=lambda item: sum(
            float(np.linalg.norm(goal-current)) for goal, current in zip(target, item)))
        safe[row, qids["left"]] = selected[0]
        safe[row, qids["right"]] = selected[1]
        modified[row] = any(np.linalg.norm(a-b) > 1e-10 for a, b in zip(selected, target))
        previous = (selected[0].copy(), selected[1].copy())
    return safe, modified


def evaluate_realized_bimanual(model, qpos, task, mapped_quat, velocity_limit_rad_s=3.14):
    actual={}; pe={}; oe={}; velocity={}
    intervals=np.r_[task.time_s[1]-task.time_s[0], np.diff(task.time_s)]
    for side in ("left", "right"):
        joint_ids=[mujoco.mj_name2id(model,mujoco.mjtObj.mjOBJ_JOINT,f"{side}_joint{i}") for i in range(1,7)]
        qids=np.asarray(model.jnt_qposadr[joint_ids],int); site=mujoco.mj_name2id(model,mujoco.mjtObj.mjOBJ_SITE,f"{side}_tcp")
        data=mujoco.MjData(model); reached=np.zeros((len(qpos),7)); position=np.zeros(len(qpos)); orientation=np.zeros(len(qpos))
        for row,state in enumerate(qpos):
            data.qpos[:]=state; mujoco.mj_forward(model,data); quat=np.empty(4); mujoco.mju_mat2Quat(quat,data.site_xmat[site])
            reached[row]=np.r_[data.site_xpos[site],quat]; position[row]=np.linalg.norm(getattr(task,f"{side}_position_m")[row]-data.site_xpos[site])
            residual=np.empty(3); mujoco.mju_subQuat(residual,mapped_quat[side][row],quat); orientation[row]=np.linalg.norm(residual)
        violations=np.zeros(len(qpos),bool); violations[1:]=np.any(np.abs(np.diff(qpos[:,qids],axis=0))/intervals[1:,None]>velocity_limit_rad_s+1e-10,axis=1)
        actual[side]=reached; pe[side]=position; oe[side]=orientation; velocity[side]=violations
    return actual,pe,oe,velocity
...
def main():
    source = load_factory_task(CSV, "insert_into_bottle")
    ...
    qpos, paired_safety_modified = enforce_paired_collision_safety(
        model, qpos, task.time_s, velocity_limit_rad_s=3.14)
    actual, pe, oe, velocity = evaluate_realized_bimanual(
        model, qpos, task, mapped_quat, velocity_limit_rad_s=3.14)
    strict_success = {side: ((pe[side] <= .001) &
                             (oe[side] <= np.deg2rad(1.5)) & ~velocity[side])
                      for side in ("left", "right")}
    ...
    for row in range(len(task.time_s)):
        ...
        if paired_safety_modified[row] and not failed:
            failed.append("paired collision avoidance modified both-arm execution")
