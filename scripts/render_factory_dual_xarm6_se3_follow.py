...
from factory_bimanual.collision_safe_follow import (
    CollisionSafeFollowConfig, DEFAULT_TIERS, solve_collision_safe_follow,
)
...
from scripts.rolling_multibranch_ik import (
    BranchCandidate, select_minimum_retime_path, select_receding_horizon_path,
)
...
OUT = ROOT / "reports/factory_bimanual/seal_bag_dual_xarm6/seal_bag_full_se3_follow_ik_v5_fixed_time_front_720p.mp4"

SELECTED_MOUNT = {
    "xy": {
        "left": (-0.31014050892839146, 0.25996862776338536),
        "right": (-0.21721384612283592, -0.34262402935409875),
    },
    "yaw": {"left": 315.0, "right": 45.0},
    "shared_base_z_m": 0.87,
    "selection_method": (
        "restored full-episode fixed-time baseline after targeted 0.9 s mount "
        "reduced global synchronous coverage; pending whole-trajectory search"),
}
...
class RunOptions:
    ...
    left_yaw_deg: float | None
    right_xy: tuple[float, float] | None
    right_yaw_deg: float | None
    shared_base_z_m: float | None
    output_stem: str
    render_video: bool
    global_retimed_no_flip: bool


def parse_run_options(argv=None):
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--robot", choices=("xarm6", "i2rt_yam", "piperx"), default="xarm6")
    parser.add_argument("--left-x", type=float)
    parser.add_argument("--left-y", type=float)
    parser.add_argument("--left-yaw", type=float)
    parser.add_argument("--right-x", type=float)
    parser.add_argument("--right-y", type=float)
    parser.add_argument("--right-yaw", type=float)
    parser.add_argument("--shared-base-z", type=float)
    parser.add_argument("--output-stem", default=OUT.stem)
    parser.add_argument("--no-video", action="store_true")
    parser.add_argument("--global-retimed-no-flip", action="store_true")
    args = parser.parse_args(argv)
    if (args.left_x is None) != (args.left_y is None):
        parser.error("--left-x and --left-y must be supplied together")
    if (args.right_x is None) != (args.right_y is None):
        parser.error("--right-x and --right-y must be supplied together")
    if not args.output_stem or Path(args.output_stem).name != args.output_stem:
        parser.error("--output-stem must be one safe filename component")
    left_xy = None if args.left_x is None else (args.left_x, args.left_y)
    right_xy = None if args.right_x is None else (args.right_x, args.right_y)
    return RunOptions(args.robot, left_xy, args.left_yaw, right_xy, args.right_yaw,
                      args.shared_base_z, args.output_stem, not args.no_video,
                      args.global_retimed_no_flip)


def output_path_for_robot(robot_name, output_stem):
    if robot_name not in ("xarm6", "i2rt_yam", "piperx"):
        raise ValueError(f"unsupported Seal_Bag robot: {robot_name}")
    return (ROOT / "reports" / "factory_bimanual" /
            f"seal_bag_dual_{robot_name}" / f"{output_stem}.mp4")


def mount_selection_method(robot_name):
    if robot_name == "piperx":
        return ("PiperX tabletop search: full coarse XY grid, 30-degree yaw sweep, "
                "then 5 cm / 15-degree / shared-height refinement; ranked by "
                "strict collision-free synchronous 6D IK coverage")
    if robot_name == "xarm6":
        return SELECTED_MOUNT["selection_method"]
    return f"{robot_name} tabletop mount override"


def use_collision_safe_pair_planner(robot_name):
    """Piper X has a physical gripper and requires paired collision planning."""
    return robot_name == "piperx"
...
def prepare_follow_targets(model, task, *, calibration=None):
    """Use the same held-quaternion reconstruction in search and full solve."""
    if calibration is not None:
        mapped = {
            side: apply_fixed_tool_rotation(
                getattr(task, f"{side}_quaternion_wxyz"),
                getattr(calibration, f"{side}_offset_quaternion_wxyz"),
            ) for side in ("left", "right")
        }
        return SimpleNamespace(**task.__dict__.copy()), mapped
    mapped = mapped_quaternions_at_joint_midpoint(model, task)
    values = task.__dict__.copy()
    prepared_quaternion = {}
    for side in ("left", "right"):
        reconstructed = reconstruct_held_quaternions(
            task.time_s, mapped[side]).quaternion_wxyz
        position, quaternion = bounded_smooth_pose_series(
            getattr(task, f"{side}_position_m"), reconstructed,
            position_cap_m=.003, orientation_cap_rad=np.deg2rad(1.0),
            sigma_frames=1.0)
        values[f"{side}_position_m"] = position
        prepared_quaternion[side] = quaternion
    return SimpleNamespace(**values), prepared_quaternion
...
def solve(model, task, *, iterations=65):
    ...
    task, mapped_quat = prepare_follow_targets(model, task)
    for side in ("left", "right"):
        joint_names = tuple(f"{side}_joint{i}" for i in range(1, 7))
...
def solve_multibranch_single_arm_method(
    ...
    global_retimed_no_flip=False, robot_name="xarm6",
    global_seed_count=12, maximum_candidates=8,
    ...
    for side in ("left", "right"):
        ...
        realized_intervals = path.required_dt_s if global_retimed_no_flip else intervals
        if count > 1:
        ...
        diagnostics[side] = {
            ...
            "required_dt_s": (path.required_dt_s.copy() if global_retimed_no_flip
                              else intervals.copy()),
...
def solve_collision_safe_bimanual_method(
        model, task, mapped_quat, *, velocity_limit_rad_s=3.0,
        robot_name="piperx", max_joint_step_rad=np.deg2rad(35.),
        candidate_iterations=60, global_seed_count=2,
        maximum_candidates_per_tier=8,
        constrained_fallback_enabled=True, strict_pose_only=False,
        stratified_refresh_interval=20,
        rolling_early_stop_candidates=2, reference_aware_enabled=False,
        orientation_adaptation_enabled=False):
    """Adapt the collision-hard paired planner to the historical result schema."""
    contract = ROBOT_CONTRACTS[robot_name]
    ...
    checker = MuJoCoPairedCollisionChecker(
        model, data, names, transition_steps=5,
        clearance_margin_m=.015 if robot_name == "piperx" else None)
    values = task.__dict__.copy()
    values.update({f"{side}_quaternion_wxyz": mapped_quat[side]
                   for side in ("left", "right")})
    if "source_row_indices" not in values:
        values["source_row_indices"] = np.arange(len(task.time_s))
    paired_task = SimpleNamespace(**values)
    periodic = []
    for side in ("left", "right"):
        joint_ids = [mujoco.mj_name2id(
            model, mujoco.mjtObj.mjOBJ_JOINT, name)
            for name in names[side]["joints"]]
        periodic.append(joint_periodic_mask(model, joint_ids))
    result = solve_collision_safe_follow(
        model, contract, paired_task,
        CollisionSafeFollowConfig(
            collision_checker=checker,
            tiers=(DEFAULT_TIERS[:1] if strict_pose_only else DEFAULT_TIERS),
            name_map=names, beam_width=64,
            maximum_pairs_per_row=64,
            maximum_candidates_per_tier=maximum_candidates_per_tier,
            candidate_iterations=candidate_iterations,
            global_seed_count=global_seed_count,
            constrained_fallback_enabled=constrained_fallback_enabled,
            stratified_seed_enabled=(robot_name == "piperx"),
            bounded_optimizer_enabled=(robot_name == "piperx"),
            wrist_risk_enabled=(robot_name == "piperx"),
            stratified_refresh_interval=stratified_refresh_interval,
            rolling_early_stop_candidates=rolling_early_stop_candidates,
            reference_aware_enabled=reference_aware_enabled,
            orientation_adaptation_enabled=orientation_adaptation_enabled,
            generate_all_tiers=False,
            # Fixed-time means every edge must fit the immutable source dt.
            max_velocity_rad_s=velocity_limit_rad_s,
            max_jump_rad=max_joint_step_rad, periodic_joints=tuple(periodic),
        ))
    if result.collision_count:
        raise RuntimeError(
            "collision-safe paired planner produced a colliding state/edge")
    count = len(task.time_s); qpos = np.zeros((count, model.nq))
    qids = {}; actual = {}; pe = {}; oe = {}; strict = {}
    discontinuity = {}; velocity = {}; diagnostics = {}
    intervals = np.r_[task.time_s[1] - task.time_s[0], np.diff(task.time_s)]
    for side, q in (("left", result.left_q), ("right", result.right_q)):
        joint_ids = [mujoco.mj_name2id(
            model, mujoco.mjtObj.mjOBJ_JOINT, name)
            for name in names[side]["joints"]]
        qids[side] = np.asarray([model.jnt_qposadr[j] for j in joint_ids], int)
        qpos[:, qids[side]] = q
        site = mujoco.mj_name2id(
            model, mujoco.mjtObj.mjOBJ_SITE, f"{side}_tcp")
        scratch = mujoco.MjData(model); reached = np.zeros((count, 7))
        position_error = np.zeros(count); orientation_error = np.zeros(count)
        for row, joints in enumerate(q):
            scratch.qpos[qids[side]] = joints; mujoco.mj_forward(model, scratch)
            quaternion = np.empty(4)
            mujoco.mju_mat2Quat(quaternion, scratch.site_xmat[site])
            reached[row] = np.r_[scratch.site_xpos[site], quaternion]
            position_error[row] = np.linalg.norm(
                getattr(task, f"{side}_position_m")[row] - scratch.site_xpos[site])
            residual = np.empty(3)
            selected_target = result.adapted_target_quaternion_wxyz[side][row]
            mujoco.mju_subQuat(residual, selected_target, quaternion)
            orientation_error[row] = np.linalg.norm(residual)
        actual[side] = reached; pe[side] = position_error; oe[side] = orientation_error
        strict[side] = ((position_error <= .001)
                        & (orientation_error <= np.deg2rad(1.5)))
        deltas = np.vstack((np.zeros(q.shape[1]), np.diff(q, axis=0)))
        discontinuity[side] = np.any(np.abs(deltas) > np.deg2rad(35.), axis=1)
        required_dt = np.maximum(
            intervals,
            np.max(np.abs(deltas), axis=1) /
            max(.8 * velocity_limit_rad_s, 1e-12))
        velocity[side] = np.zeros(count, dtype=bool)
        tiers = result.left_tier if side == "left" else result.right_tier
        diagnostics[side] = {
            "candidate_count": result.candidate_count[side].copy(),
            "collision_free_candidate_count": result.safe_pair_count.copy(),
            "selected_candidate_id": np.asarray([
                pair[0 if side == "left" else 1] if pair is not None else -1
                for pair in result.paired_branch_indices], int),
            "recovery_mode": result.recovery_mode.copy(),
            "required_dt_s": required_dt,
            "selected_joint_limit_margin_rad": (
                result.selected_joint_limit_margin_rad[side].copy()),
            "selected_singularity_margin": (
                result.selected_singularity_margin[side].copy()),
            "tolerance_tier": tiers.copy(),
            "orientation_adaptation_level": (
                result.orientation_adaptation_level[side].copy()),
            "adapted_target_quaternion_wxyz": (
                result.adapted_target_quaternion_wxyz[side].copy()),
            "tool_axis_offset_deg": result.tool_axis_offset_deg[side].copy(),
            "swing_offset_deg": result.swing_offset_deg[side].copy(),
        }
    diagnostics["paired"] = result
    return (qpos, actual, pe, oe, strict, discontinuity, velocity,
            diagnostics)
...
def main(argv=None):
    ...
    run_out = output_path_for_robot(options.robot_name, options.output_stem)
    run_out.parent.mkdir(parents=True, exist_ok=True)
    contract = ROBOT_CONTRACTS[options.robot_name]
    ...
    best["selection_method"] = mount_selection_method(options.robot_name)
    best["xy"] = dict(best["xy"])
    best["yaw"] = dict(best["yaw"])
    if options.left_xy is not None:
        best["xy"]["left"] = options.left_xy
    if options.left_yaw_deg is not None:
        best["yaw"]["left"] = options.left_yaw_deg
    if options.right_xy is not None:
        best["xy"]["right"] = options.right_xy
    if options.right_yaw_deg is not None:
        best["yaw"]["right"] = options.right_yaw_deg
    if options.shared_base_z_m is not None:
        best["shared_base_z_m"] = options.shared_base_z_m
    ...
    scene = run_out.with_suffix(".scene.xml")
    ...
    task, mapped_quat = prepare_follow_targets(model, task)
    velocity_limit = 3.0 if options.robot_name == "piperx" else 3.14
    if use_collision_safe_pair_planner(options.robot_name):
        (qpos, actual, pe, oe, strict_success, discontinuity, velocity,
         solver_diagnostics) = solve_collision_safe_bimanual_method(
        ...
        (qpos, actual, pe, oe, strict_success, discontinuity, velocity,
         solver_diagnostics) = solve_multibranch_single_arm_method(
            model, task, mapped_quat, horizon=12, beam_width=8,
            candidate_iterations=60, velocity_limit_rad_s=velocity_limit,
            global_retimed_no_flip=options.global_retimed_no_flip,
            robot_name=options.robot_name)
    ...
    collision, state_collision_classes, edge_collision_classes = \
        audit_bimanual_collisions(model, qpos, robot_name=options.robot_name)
    paired_result = solver_diagnostics.get("paired")
    if paired_result is not None and (
            paired_result.collision_count or np.any(collision)):
        raise RuntimeError(
            "collision-hard paired planner produced a colliding state/edge")
    ...
    if options.render_video:
        ...
        rendered = render_mujoco_mp4(scene, run_out, execution_time_s, qpos=qpos, diagnostics=diagnostics,
    ...
    if paired_result is None:
        left_tiers = np.where(strict["left"], "exact", "hold")
        right_tiers = np.where(strict["right"], "exact", "hold")
    else:
        left_tiers = paired_result.left_tier
        right_tiers = paired_result.right_tier
    summary = {"mode": "full_SE3_position_and_quaternion_IK", "robot": options.robot_name,
        ...
        "controller_joint_velocity_limit_rad_s": velocity_limit,
        ...
        "solver": {"candidate_iterations": 60, "maximum_candidates": 8,
                   "path_selection": ("global minimum retime with visual wrist flip gate"
                                      if options.global_retimed_no_flip else
                                      "fixed-source-time rolling branch continuity"),
